...
def create_flickr_table():
    ...
    insert = '''
        ...
        VALUES (NULL,?,?,?,?,?,?)
        '''
    with open('flickr_results.csv', 'r') as infile:
...
def create_twitter_table():
    ...
    insert = '''
        ...
        VALUES (NULL,?,?,?,?,?,?)
        '''
    with open('twitter_results/twitter_results_master.csv', 'r', encoding='ISO-8859-1') as infile:
        reader = csv.reader(infile)
        next(reader)
        for row in reader:
            params = (row[0], row[1], row[2], row[3], row[4], row[5])
...
def create_youtube_table():
    ...
    insert = '''
        ...
        VALUES (NULL,?,?,?,?,?,?)
        '''
    with open('youtube_results.csv', 'r') as infile:
        reader = csv.reader(infile)
        next(reader)
        for row in reader:
            params = (row[0], row[1], row[2], row[3], row[4], row[5])
